...
*  **メール校正の着想はエンジニアの校正ツールとも言えるIDEから**
   * **文章校正**
   コードのデバッグやコンパイルのように，メールを作成した際に自動で校正や敬語変換を行い，丁寧な表現に変換ができる．ON/OFFで切り替え可能なためカジュアルなメール送信も可能です。
